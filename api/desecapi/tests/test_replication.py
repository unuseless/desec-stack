...
class RepositoryTest(testcases.TestCase):
    ...
    def assertHead(self, repo, message=None, sha=None):
        actual_message, actual_sha = repo.get_head()
    ...
    def assertHasCommit(self, repo: Repository, commit_id):
        self.assertNotEqual(
            repo.get_commit(commit_id)[0], None,
            f'Expected repository to have commit {commit_id}, but it had not.'
        )
    ...
    def assertHasNotCommit(self, repo: Repository, commit_id):
        self.assertEqual(
            repo.get_commit(commit_id)[1], None,
            f'Expected repository to not have commit {commit_id}, but it had.'
        )
    ...
    def test_remove_history(self):
        with TemporaryDirectory() as path:
            ...
            repo.remove_history(before=timedelta(seconds=2))
